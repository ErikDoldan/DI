...
public class HomeFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        // ✅  AQUÍ INSTANCIAMOS EL AUTHVIEWMODEL Y OBSEVAMOS SU EVENTO,
        // EN EL ONCLICK SOLO LE PASAMSO AL MODELO
        AuthViewModel authVm = new ViewModelProvider(
                requireActivity(),
                new AuthViewModelFactory(new AuthRepository())
        ).get(AuthViewModel.class);


        authVm.getNavEvent().observe(getViewLifecycleOwner(), event -> {
            ...
            authVm.consumeNavEvent();
        });

        btnLogout.setOnClickListener(v -> authVm.logout());
        btnDeleteLast.setOnClickListener(v -> noticeVm.deleteLast());

        view.findViewById(R.id.btnVerPerfil).setOnClickListener(v -> {
            NavHostFragment.findNavController(this)
                    .navigate(R.id.action_home_to_profile);
        });
        //ANTES LA CONEXION CON FIREBASE DEBERIA HACERSE SIGUIENDO PATRON MVVM
